...
class World:
                ...
                for index_OF_column, column in enumerate(row):
                    if column != '-1': #ma rysować granice tylko w polach, które nie są puste (-1), tutaj wartość 395 oznacza granicę
                        ...
                        if type == 'entities':#1 skeletor,2 grzybol, 3 pso niedzwiedz
                            if column == '0':
                                self.froggo = Froggo((x,y), [self.visable_sprites], self.obstacle_sprites, self.lava_sprites,self.teleporter_1_sprite,self.teleporter_2_sprite,self.teleporter_3_sprite,self.attack, self.kill_weapon_sprite, self.create_spells)
                            else:
                                if column == '1': 
                                ...
                                Monster(monster_name,(x,y),[self.visable_sprites,self.destroyable_sprites], self.obstacle_sprites, self.lava_sprites,self.damage_froggo, self.trigger_death_elements, self.add_score_xp)
    ...
    def add_score_xp(self,amount):
        ...
        if self.froggo.monster_count == 13:
